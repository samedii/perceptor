...
def make_cond_model_fn(model, cond_fn):
    def cond_model_fn(x, t, **extra_args):
        ...
        wanted_pred = cond_fn(x, t, pred, **extra_args).detach()
        # pretend model predicted another velocity
        wanted_v = (x * alphas[:, None, None, None] - wanted_pred) / sigmas[
            :, None, None, None
        ]
        # could also do v.requires_grad_(True)
        # another alternative is to "keep eps"
        return wanted_v
...
class DiffusionIdea(DrawingInterface):
    ...
    def inverse_(self, guide, n_steps):
        ...
        def cond_fn(x, t, pred, clip_embed=None):
            ...
            wanted_pred = guide((pred + 1) / 2)
            return wanted_pred * 2 - 1
