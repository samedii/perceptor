...
class OwlViTClassPredictionHead(nn.Module):
    ...
    def forward(
        ...
        # TODO: consider replacing with spherical distance
...
class OwlViTForObjectDetection(OwlViTPreTrainedModel):
    ...
    def box_predictor(
        ...
        pred_boxes = pred_boxes + self.compute_box_bias(feature_map).to(pred_boxes)
    ...
    def forward(
        ...
        if not return_dict:
            return (
                ...
                query_embeds.to(self.device),
                feature_map.to(self.device),
                class_embeds.to(self.device),
                text_model_last_hidden_states,
                vision_model_last_hidden_states,
            )

        return OwlViTObjectDetectionOutput(
            image_embeds=feature_map.to(self.device),
            text_embeds=query_embeds.to(self.device),
            pred_boxes=pred_boxes.to(self.device),
            logits=pred_logits.to(self.device),
            class_embeds=class_embeds.to(self.device),
